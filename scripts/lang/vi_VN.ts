/**
 * Vietnamese Language File for APEIRIX
 * AUTO-GENERATED from configs/script-lang/vi_VN/*.yaml
 * DO NOT EDIT - Changes will be overwritten
 */

export const LANG_VI = {
    "ui": {
        "close": "§l§c✖ Đóng",
        "back": "§l§0← Quay lại",
        "backToList": "§l§0← Quay lại Danh sách",
        "backToMenu": "§l§0← Quay lại Menu"
    },
    "achievements": {
        "title": "§l§6═══ THÀNH TỰU APEIRIX ═══",
        "selectCategory": "§0Chọn nhánh thành tựu để xem:",
        "progress": "§0Tiến độ:",
        "completed": "§8hoàn thành",
        "status": "§0Trạng thái:",
        "statusCompleted": "§2§lĐÃ HOÀN THÀNH",
        "statusLocked": "§8§lCHƯA MỞ KHÓA",
        "rewardClaim": "§eNhận",
        "rewardClaimed": "§a✓ Đã nhận",
        "rewardLocked": "§8✗ Chưa mở khóa",
        "congratulations": "§2§l★ CHÚC MỪNG! ★\n§0Bạn đã hoàn thành thành tựu này!",
        "keepGoing": "§6§l⚡ CỐ GẮNG LÊN! ⚡\n§0Bạn sắp hoàn thành rồi!",
        "unlocked": "§6§l★ MỞ KHÓA THÀNH TỰU! ★",
        "received": "§a§l+ Nhận được:"
    },
    "categories": {
        "metallurgy": {
            "name": "§6⚒ LUYỆN KIM",
            "desc": "§0Thế giới vật chất - Quặng → Hợp kim → Công cụ/Giáp"
        },
        "starter": {
            "name": "§a✦ KHỞI ĐẦU",
            "desc": "§0Những bước đầu tiên trong thế giới APEIRIX"
        },
        "food": {
            "name": "§eTHỰC PHẨM & SINH TỒN",
            "desc": "§0Đóng hộp, bảo quản và dinh dưỡng"
        },
        "processing": {
            "name": "§b⚙ CHẾ BIẾN",
            "desc": "§0Máy móc, tự động hóa và hiệu suất"
        }
    },
    "phases": {
        "metallurgy": {
            "phase1": {
                "name": "§6Phase 1: §0Thời Đại Đồng Thau",
                "desc": "§0Làm quen với cơ chế luyện kim và hợp kim cơ bản"
            }
        }
    },
    "achievementNames": {
        "welcome": "Chào Mừng",
        "first_mine": "Khai Thác Đầu Tiên",
        "first_craft": "Chế Tạo Đầu Tiên",
        "first_ore": "Quặng Đầu Tiên",
        "tin_collector": "Thợ Thu Thập Thiếc",
        "bronze_maker": "Thợ Rèn Đồng Thau",
        "alloy_master": "Bậc Thầy Hợp Kim",
        "tool_crafter": "Thợ Chế Tạo Công Cụ",
        "armor_smith": "Thợ Rèn Giáp",
        "crusher_user": "Người Vận Hành Máy Nghiền",
        "hammer_expert": "Chuyên Gia Búa",
        "bronze_age_complete": "Hoàn Thành Thời Đại Đồng Thau",
        "first_can": "Hộp Đầu Tiên",
        "can_collector": "Thợ Thu Thập Hộp",
        "recycler": "Người Tái Chế",
        "processing_master": "Bậc Thầy Chế Biến",
        "dust_collector": "Thợ Thu Thập Bụi",
        "efficiency_expert": "Chuyên Gia Hiệu Suất"
    },
    "achievementDescs": {
        "welcome": "§0Tham gia thế giới APEIRIX lần đầu tiên",
        "first_mine": "§0Khai thác khối đầu tiên của bạn",
        "first_craft": "§0Chế tạo vật phẩm đầu tiên của bạn",
        "first_ore": "§0Khai thác quặng thiếc đầu tiên",
        "tin_collector": "§0Thu thập 64 quặng thiếc",
        "bronze_maker": "§0Tạo ra thỏi đồng thau đầu tiên",
        "alloy_master": "§0Tạo 32 thỏi đồng thau",
        "tool_crafter": "§0Chế tạo bộ công cụ đồng thau hoàn chỉnh",
        "armor_smith": "§0Chế tạo bộ giáp đồng thau hoàn chỉnh",
        "crusher_user": "§0Sử dụng máy nghiền quặng lần đầu",
        "hammer_expert": "§0Sử dụng búa để khai thác 100 blocks",
        "bronze_age_complete": "§0Hoàn thành tất cả thành tựu Phase 1",
        "first_can": "§0Chế tạo hộp rỗng đầu tiên",
        "can_collector": "§0Thu thập 10 loại thực phẩm đóng hộp khác nhau",
        "recycler": "§0Rửa 50 hộp bẩn",
        "processing_master": "§0Sử dụng tất cả 7 máy chế biến",
        "dust_collector": "§0Thu thập 100 bụi tinh khiết",
        "efficiency_expert": "§0Chế biến 1000 vật phẩm qua máy móc"
    },
    "welcome": {
        "title": "§a§lChào mừng đến với APEIRIX!",
        "firstTime": "§e§lBạn đã nhận được Sách Thành Tựu APEIRIX!\n§0Sử dụng sách để xem tiến độ thành tựu của bạn",
        "returning": "§0Nếu làm mất sách, gõ: §b/scriptevent apeirix:getbook",
        "bookReceived": "§a§lĐã nhận lại Sách Thành Tựu APEIRIX!"
    },
    "items": {
        "achievementBook": "Sách Thành Tựu APEIRIX"
    },
    "wiki": {
        "title": "§l§0BÁCH KHOA TOÀN THƯ",
        "itemCount": "§0Đã khám phá: §0{count}§0 vật phẩm",
        "noItemsFound": "§cKhông tìm thấy vật phẩm APEIRIX nào trong túi đồ!",
        "selectItem": "§0Chọn vật phẩm để xem chi tiết",
        "back": "§cQuay lại",
        "emptyTitle": "§l§0BÁCH KHOA TOÀN THƯ",
        "emptyMessage": "§0Bạn chưa có vật phẩm APEIRIX nào trong túi đồ.\n\n§0Hãy khám phá thế giới và thu thập các vật phẩm để mở khóa thông tin trong bách khoa toàn thư!\n\n§0Các danh mục có sẵn:\n§0• §0Nguyên liệu\n§0• §0Công cụ\n§0• §0Giáp\n§0• §0Thức ăn\n§0• §0Đặc biệt",
        "emptyButton": "§0Đã hiểu",
        "description": "§l§0MÔ TẢ:",
        "category": "§0Danh mục:",
        "information": "§l§0Thông tin",
        "categories": {
            "materials": "§0Nguyên liệu",
            "tools": "§0Công cụ",
            "armor": "§0Giáp",
            "foods": "§0Thức ăn",
            "special": "§0Đặc biệt",
            "blocks": "§0Khối",
            "ores": "§0Quặng",
            "entities": "§0Sinh vật"
        }
    },
    "attributes": {
        "breakable": "§cGẫy",
        "durability": "§6Độ bền",
        "combat_damage": "§6Sát thương",
        "damage_multiplier": "§6Sát thương tăng",
        "mining_speed": "§6Tốc độ đào",
        "hammer_mining": "§6Đập Búa",
        "rust_mite_edible": "§cRust Mite"
    }
};
