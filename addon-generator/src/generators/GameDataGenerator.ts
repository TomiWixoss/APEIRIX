...
export interface BlockInfoData {
  ...
  displayName: string; // Tên hiển thị đã resolve từ lang file
}
...
export class GameDataGenerator {
   ...
   * @param allItems All item IDs for Rust Mite edible items
   * @param edibleItems Items explicitly marked as edible by Rust Mite
   * @param blocks Block info data array
   * @param armor Armor data array
   * @param entities Entity attribute data array
   * @param customPath Optional custom output path (relative to projectRoot)
   */
  generate(tools: ToolData[], foods: FoodData[], ores: OreData[], wikiItems: WikiItemData[], hammerMining: HammerMiningData[], brassSifter: BrassSifterData[], allItems: string[], edibleItems?: string[], blocks?: BlockInfoData[], armor?: ArmorData[], entities?: EntityAttributeData[], customPath?: string): void {
    const content = this.generateContent(tools, foods, ores, wikiItems, hammerMining, brassSifter, allItems, edibleItems || [], blocks || [], armor || [], entities || []);
   ...
   */
  private generateContent(tools: ToolData[], foods: FoodData[], ores: OreData[], wikiItems: WikiItemData[], hammerMining: HammerMiningData[], brassSifter: BrassSifterData[], allItems: string[], edibleItems: string[], blocks: BlockInfoData[], armor: ArmorData[], entities: EntityAttributeData[]): string {
    return `/**
...
`;
  ...
  /**
   * Generate Rust Mite edible items
   * YAML-DRIVEN: Only includes items explicitly marked with edibleByRustMite: true
   */
  private generateRustMiteEdibleItems(edibleItems: string[]): string {
    if (edibleItems.length === 0) {
      return '  // No edible items generated';
    }
   ...
   * Generate armor data
   */
  private generateArmorData(armor: ArmorData[]): string {
    if (armor.length === 0) {
      return '  // No armor generated';
    }
  ...
  /**
   * Generate block info data
   */
  private generateBlockData(blocks: BlockInfoData[]): string {
    if (blocks.length === 0) {
    ...
    return blocks.map(block => {
      ...
      lines.push(`  {`);
      lines.push(`    blockId: "${block.blockId}",`);
      lines.push(`    blockType: "${block.blockType}",`);
      
      if (block.machineType) {
        lines.push(`    machineType: "${block.machineType}",`);
      }
      
      lines.push(`    displayName: "${block.displayName}"`);
      lines.push(`  }`);
      return lines.join('\n');
    }).join(',\n');
  }
  ...
  /**
   * Generate entity attribute data
   */
  private generateEntityData(entities: EntityAttributeData[]): string {
    ...
    return entities.map(entity => {
      const lines: string[] = [];
      lines.push(`  {`);
      lines.push(`    entityId: "${entity.entityId}",`);
