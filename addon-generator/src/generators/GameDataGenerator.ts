...
export class GameDataGenerator {
   ...
   */
  generate(tools: ToolData[], foods: FoodData[], ores: OreData[], wikiItems: WikiItemData[], hammerMining: HammerMiningData[], brassSifter: BrassSifterData[], allItems: string[], customPath?: string): void {
    const content = this.generateContent(tools, foods, ores, wikiItems, hammerMining, brassSifter, allItems);
   ...
   */
  private generateContent(tools: ToolData[], foods: FoodData[], ores: OreData[], wikiItems: WikiItemData[], hammerMining: HammerMiningData[], brassSifter: BrassSifterData[], allItems: string[]): string {
    return `/**
...
${this.generateRustMiteEdibleItems(foods, ores, allItems)}
];
`;
  }
   ...
   * Includes: all foods, raw ores, and metal dusts
   */
  private generateRustMiteEdibleItems(foods: FoodData[], ores: OreData[], allItems: string[]): string {
    const edibleItems = new Set<string>();

    // Add all food items
    foods.forEach(food => edibleItems.add(food.id));

    // Add raw ores from ore data
    ores.forEach(ore => {
      if (ore.dropItem) {
        edibleItems.add(ore.dropItem);
      }
    });

    // Add vanilla raw ores
    edibleItems.add('minecraft:raw_iron');
    edibleItems.add('minecraft:raw_copper');
    edibleItems.add('minecraft:raw_gold');

    // Add all items that contain "dust" in their ID
    allItems.forEach(itemId => {
      if (itemId.includes('dust') || itemId.includes('candirty') || itemId.includes('canempty')) {
        edibleItems.add(itemId);
      }
    });
